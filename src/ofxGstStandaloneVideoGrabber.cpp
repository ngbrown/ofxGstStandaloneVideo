/*
 * ofxGstStandaloneVideoGrabber.cpp
 *
 */
...
static void get_video_devices (ofxGstStandaloneCamData & cam_data)
{
#ifdef TARGET_LINUX
	int fd, ok;
	...
	udev_list_entry_foreach(entry,list){
		...
		ofxGstStandaloneDevice gst_device;
...
static void get_supported_framerates (ofxGstStandaloneVideoFormat &video_format, GstStructure &structure)
{
	const GValue *framerates;
	ofxGstStandaloneFramerate framerate;
...
static void find_framerate (ofxGstStandaloneVideoFormat &format, int desired_framerate = -1)
{
...
static int find_resolution(ofxGstStandaloneDevice &webcam_device, int width, int height){
...
static void add_video_format (ofxGstStandaloneDevice &webcam_device,
  ofxGstStandaloneVideoFormat &video_format, GstStructure &format_structure, int desired_framerate)
{
...
static void get_supported_video_formats (ofxGstStandaloneDevice &webcam_device, GstCaps &caps, int desired_framerate)
{
	...
	for (int i = 0; i < num_structures; i++){
		...
		if (G_VALUE_HOLDS_INT (width)){
			ofxGstStandaloneVideoFormat video_format;
...
static void get_device_data (ofxGstStandaloneDevice &webcam_device, int desired_framerate)
{
    ...
    if ((pipeline == NULL) || (err != NULL)){
    	if (err){
    		ofLog(OF_LOG_ERROR, "ofxGstStandaloneUtils: error getting device data: %s", err->message);
    		g_error_free (err);
    	}else{
    		ofLog(OF_LOG_ERROR, "ofxGstStandaloneUtils: error getting device data, cannot get pipeline");
    	}
    	if(pipeline)
	...
	if ((msg == NULL) && (ret == GST_STATE_CHANGE_SUCCESS)){
		...
		ofLog(OF_LOG_ERROR, "ofxGstStandaloneUtils: error getting device data; module %s reported: %s",
...
ofxGstStandaloneVideoGrabber::ofxGstStandaloneVideoGrabber(){
	bIsCamera			= false;
...
void ofxGstStandaloneVideoGrabber::setVerbose(bool bVerbose){
	//if(bVerbose) ofLogSetTopicLogLevel("ofxGstStandaloneVideoGrabber",OF_LOG_VERBOSE);
...
void ofxGstStandaloneVideoGrabber::setDeviceID(int id){
	...
	if(int(camData.webcam_devices.size())>id){
		...
		ofLog(OF_LOG_WARNING,"ofxGstStandaloneUtils: selected device doesn't exist, selecting device 0");
	}
}

ofxGstStandaloneVideoFormat & ofxGstStandaloneVideoGrabber::selectFormat(int w, int h, int desired_framerate){
	int minDiff=999999;
...
bool ofxGstStandaloneVideoGrabber::initGrabber(int w, int h){
	if(!camData.bInited) get_video_devices(camData);

	if(camData.webcam_devices.size()==0){
		ofLog(OF_LOG_ERROR,"ofxGstStandaloneUtils: no devices found exiting without initializing");
		return false;
	}

	ofxGstStandaloneVideoFormat & format = selectFormat(w, h, attemptFramerate);
	ofLog(OF_LOG_NOTICE,"ofxGstStandaloneUtils: selected device: " + camData.webcam_devices[deviceID].product_name);
	ofLog(OF_LOG_NOTICE,"ofxGstStandaloneUtils: selected format: " + ofToString(format.width) + "x" + ofToString(format.height) + " " + format.mimetype + " framerate: " + ofToString(format.choosen_framerate.numerator) + "/" + ofToString(format.choosen_framerate.denominator));
...
void ofxGstStandaloneVideoGrabber::setDesiredFrameRate(int framerate){
	attemptFramerate = framerate;
}

ofxGstStandaloneVideoUtils * ofxGstStandaloneVideoGrabber::getGstVideoUtils(){
	return &videoUtils;
}


void ofxGstStandaloneVideoGrabber::update(){
	videoUtils.update();
}

bool ofxGstStandaloneVideoGrabber::isFrameNew(){
	return videoUtils.isFrameNew();
}

unsigned char * ofxGstStandaloneVideoGrabber::getPixels(){
	return videoUtils.getPixels();
}

ofPixelsRef ofxGstStandaloneVideoGrabber::getPixelsRef(){
	return videoUtils.getPixelsRef();
}

float ofxGstStandaloneVideoGrabber::getHeight(){
	return videoUtils.getHeight();
}

float ofxGstStandaloneVideoGrabber::getWidth(){
	return videoUtils.getWidth();
}

void ofxGstStandaloneVideoGrabber::close(){
	videoUtils.close();
}
